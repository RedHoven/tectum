...
import { ArrowLeft, Check, Lightbulb, Zap, Activity } from 'lucide-react';
...
const IconPanel = () => (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="drop-shadow-sm">
    ...
    <line x1="9" y1="4" x2="9" y2="20" /><line x1="15" y1="4" x2="15" y2="20" /><line x1="3" y1="12" x2="21" y2="12" />
  </svg>
);
const IconBattery = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="drop-shadow-sm">
    ...
    <line x1="9" y1="2" x2="15" y2="2" /><line x1="10" y1="13" x2="14" y2="13" /><line x1="12" y1="11" x2="12" y2="15" />
  </svg>
);
const IconHeatPump = () => (
  <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="drop-shadow-sm">
    ...
    <circle cx="15" cy="12" r="4" /><line x1="6" y1="9" x2="9" y2="9" /><line x1="6" y1="12" x2="9" y2="12" /><line x1="6" y1="15" x2="9" y2="15" />
  </svg>
);
const IconWallbox = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="drop-shadow-sm">
    <rect x="5" y="4" width="9" height="15" rx="2" fill="currentColor" fillOpacity="0.15"/>
    <circle cx="9.5" cy="11.5" r="2.5" /><path d="M14 14c3 0 5-1 5-4v-1" /><rect x="18" y="7" width="2" height="4" rx="1" />
  </svg>
);
...
export function PlannerPro({ intake, onBack, projectName }: PlannerProProps) {
  ...
  const activeScenarioId = useMemo(() =>
    scenarios.find(s =>
      s.cfg.panelCount === cfg.panelCount && s.cfg.batteryKwh === cfg.batteryKwh &&
      s.cfg.includeHeatPump === cfg.includeHeatPump && s.cfg.includeWallbox === cfg.includeWallbox
    )?.id, [cfg, scenarios]);
  ...
  return (
    <div className="h-screen flex flex-col bg-background overflow-hidden relative">
      {/* Header */}
      <header className="h-[60px] flex-none border-b bg-card flex items-center px-6 justify-between z-10">
        <div className="flex items-center gap-3">
          <img src="/logo.png" alt="Tectum" className="w-8 h-8" />
          <span className="font-display text-2xl">tectum</span>
          {projectName && (
            <span className="text-[13px] text-muted-foreground ml-2">/ {projectName}</span>
          )}
        </div>
        <div className="text-sm text-muted-foreground font-medium">{intake.address}</div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 relative overflow-hidden">
        ...
        <div className="col-span-1 lg:col-span-2 relative bg-gradient-sky flex flex-col overflow-hidden">
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
            <div className="bg-white/90 backdrop-blur-md p-1.5 rounded-xl shadow-[0_10px_30px_rgba(45,55,119,0.15)] border border-white flex gap-1.5">
              {[
                { active: cfg.panelCount > 0, icon: IconPanel, count: cfg.panelCount > 0 ? cfg.panelCount : null },
                { active: cfg.batteryKwh > 0, icon: IconBattery, count: cfg.batteryKwh > 0 ? cfg.batteryKwh : null },
                { active: cfg.includeHeatPump, icon: IconHeatPump, count: null },
                { active: cfg.includeWallbox, icon: IconWallbox, count: null },
              ].map((item, i) => (
                <div key={i} className={cn("relative w-14 h-14 rounded-lg border transition-all flex items-center justify-center",
                  item.active ? "bg-primary border-primary text-white" : "bg-secondary border-border text-foreground/60"
                )}>
                  <item.icon />
                  {item.count !== null && <span className="absolute bottom-1 right-1 text-white text-[10px] font-bold">{item.count}</span>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="h-full overflow-y-auto scrollbar-hide bg-background border-l z-20 flex flex-col w-full">
          <div className="p-8 flex-1 flex flex-col gap-10">
            ...
            <section>
              <div className="text-[11px] font-semibold tracking-[0.1em] uppercase text-muted-foreground mb-3">Recommended</div>
            ...
            <section className="space-y-4">
              <div className="text-[11px] font-semibold tracking-[0.1em] uppercase text-muted-foreground">Compare Scenarios</div>
              <div className="flex flex-col gap-3">
                {scenarios.map(s => {
                  const isActive = activeScenarioId === s.id;
                  return (
                    <button key={s.id} onClick={() => setCfg(s.cfg)}
                      className={cn(
                        "w-full text-left p-5 rounded-2xl border transition-all relative overflow-hidden bg-card",
                        isActive ? "border-primary shadow-[0_0_0_1px_hsl(232_45%_32%)]" : "border-border shadow-sm hover:border-gray-300"
                      )}
                    >
                      ...
                      <div className="flex justify-between items-baseline mt-1">
                        ...
                        <span className="text-[13px] font-medium">{s.yld.paybackYears} y <span className="text-muted-foreground">·</span> saves {fmtEUR(s.yld.yearlySavings)}/y</span>
                      </div>
                    </button>
                  );
                })}
            ...
            <AnimatePresence>
              {tips.length > 0 && (
                <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
                  <div className="text-[11px] font-semibold tracking-[0.1em] uppercase text-muted-foreground flex items-center gap-2">
                    <Lightbulb className="w-4 h-4" /> Smart tips
                  </div>
                  {tips.map((tip, i) => (
                    <div key={i} className="bg-[#FAF9F5] border px-5 py-4 rounded-2xl text-[14px] leading-relaxed">{tip}</div>
                  ))}
                </motion.section>
              )}
            </AnimatePresence>
            ...
            <section className="space-y-6 pt-2">
              <div className="text-[11px] font-semibold tracking-[0.1em] uppercase text-muted-foreground">Refine your design</div>
              <div className="space-y-3">
                ...
                <Slider min={4} max={maxPanels} step={1} value={[cfg.panelCount]}
                  onValueChange={v => setCfg(p => ({...p, panelCount: v[0]}))}
                  className="py-1 [&_[role=slider]]:h-5 [&_[role=slider]]:w-5 [&_[role=slider]]:border-[3px]" />
                <div className="flex justify-between text-[13px] text-muted-foreground pt-1"><span>4</span><span>max {maxPanels}</span></div>
              </div>
              <div className="space-y-3 pt-2">
                ...
                <Slider min={0} max={20} step={2.5} value={[cfg.batteryKwh]}
              ...
              <div className="pt-2 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center shrink-0"><Zap className="w-5 h-5 text-muted-foreground" /></div>
                    <div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center shrink-0"><Activity className="w-5 h-5 text-muted-foreground" /></div>
                    <div>
            ...
            <section className="space-y-4 pt-2">
              <div className="text-[11px] font-semibold tracking-[0.1em] uppercase text-muted-foreground mb-4">Cost breakdown</div>
              {[
                { label: 'Solar panels', value: cfg.panelCount * 250 },
                { label: 'Inverter', value: costs.inverter },
                { label: 'Battery', value: costs.battery },
                { label: 'Installation', value: costs.installation },
              ].map(item => (
                <div key={item.label} className="flex justify-between text-[15px] text-muted-foreground">
                  <span>{item.label}</span>
                  <span className="text-foreground font-medium">{item.value.toLocaleString('de-DE')} €</span>
                </div>
              ))}
          ...
          <div className="bg-primary text-primary-foreground p-8">
            ...
            >
              {({ loading }) => (
                <button className="w-full bg-white text-primary hover:bg-gray-100 h-12 text-[16px] font-semibold rounded-xl transition-colors">
